...
void gd_eval_com_init(uint32_t com)
{
    ...
    usart_baudrate_set(EVAL_COM0,115200U);                    //设置波特率
    usart_parity_config(EVAL_COM0,USART_PM_NONE);             //无奇偶校验
...
void USART0_IRQHandler(void)
{
    ...
    {
        usart_interrupt_flag_clear(EVAL_COM0,USART_INT_FLAG_RBNE); /* 清除空闲中断标志位 */
        usart_data_receive(EVAL_COM0);                             /* 清除接收完成标志位 */
        usart_flag_get(EVAL_COM0, USART_FLAG_TC);                   /* 清除标志位 */
